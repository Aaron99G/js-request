...
sayHelloButton.addEventListener('mouseout',reverseColor);
...
const ohMy = () => {
    ...
    .then(function(res) {
        for(let i =0; i < res.data.length; i++) { 
            let oob = document.createElement('p').textContent = res.data[i];
            document.querySelector('body').appendChild(oob);
        }
    })
    .catch(error => {
        console.log(error)
...
let repBtn = document.querySelector('#repeat-button');
repBtn.addEventListener("click",repeatMyParam);
...
document.querySelector('#query-button').addEventListener("click",test1);
